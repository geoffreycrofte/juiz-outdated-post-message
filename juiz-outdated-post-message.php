...
define( 'JUIZ_ODPM_LANG',			'jodpm_lang' );
...
function make_juiz_odpm_multilang() {
	load_plugin_textdomain( JUIZ_ODPM_LANG, false, JUIZ_ODPM_DIRNAME.'/languages' );
}
...
if (!function_exists('get_odpm_outdated_message')) {
	function get_odpm_outdated_message() {
		global $post;
		...
		if ( $show_me ) {
			...
			if ( in_array(get_post_type($post->ID), $post_types) && $where !== 'nowhere' && $days_between >= $delay ) {
				...
				$message_sc  = '<'.$div.' class="juiz-outdated-message'.$classes.'">'.$before.$message.$after.'</'.$div.'>';
				$message_top = '<'.$div.' class="juiz-outdated-message'.$classes.$classes_top.'"'.$id_top.'>'.$before.$message.$after.'</'.$div.'>';
				$message_bott= '<'.$div.' class="juiz-outdated-message'.$classes.$classes_bott.'"'.$id_bott.'>'.$before.$message.$after.'</'.$div.'>';

				return array("message_top" => $message_top, "message_bottom" => $message_bott, "message_sc" => $message_sc, "where"=> $where );
...
if ( !function_exists('juiz_odpm_shortcode')) {
	function juiz_odpm_shortcode( $atts ){
		...
		$message = get_odpm_outdated_message();
...
// some add_filter examples
/*
function add_content_before($content) {
	return '<i class="icon-date" aria-hidden="true"></i>';
}
add_filter('juiz_odpm_before_message', "add_content_before");
*/